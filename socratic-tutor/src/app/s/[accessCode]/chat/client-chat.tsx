...
export function ClientChat({ accessCode, sessionName, maxExchanges }: ClientChatProps) {
  ...
  const exchangeCount = Math.ceil(messages.filter(m => m.role === "user").length);
  ...
  useEffect(() => {
    ...
    if (!initialized.current && messages.length === 0) {
      ...
      sendMessage(`Hi. My name is ${sname || "a student"}. I'm ready to begin the session. Please greet me by name and give me a thought-provoking opening question about the readings to get us started. Do not answer it yourself.`, sid);
    }
  ...
  const sendMessage = async (contentToSend: string, sid: string | null = studentSessionId) => {
    if (!sid || !contentToSend.trim() || isLoading) return;
    
    setIsLoading(true);
    setError(false);

    const userMessage: Message = { id: Math.random().toString(), role: "user", content: contentToSend };
        ...
        if (value) {
          ...
          setMessages((prev) => {
            ...
            if (mapped[lastIdx].id === assistantMessageId) {
              mapped[lastIdx] = { ...mapped[lastIdx], content: mapped[lastIdx].content + chunk };
            }
            return mapped;
          });
        }
  ...
  const handleEndClick = () => {
    if (studentSessionId && confirm("Are you sure you want to end this tutoring session?")) {
          ...
          <div className="max-w-4xl mx-auto flex flex-col items-center">
            ...
            {(exchangeCount >= maxExchanges && !isLoading && !isEnded) && (
               <div className="mt-4 text-center">
                 <p className="text-sm font-medium text-amber-600 mb-2">You have reached the exchange limit for this session.</p>
                 <button onClick={() => triggerEndSession(studentSessionId!)} className="px-4 py-2 bg-amber-100 hover:bg-amber-200 text-amber-700 rounded-lg text-sm transition-colors">View Summary</button>
               </div>
            )}
          </div>
        </div>
